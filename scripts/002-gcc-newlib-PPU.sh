...
GCC="gcc-7.2.0"
NEWLIB="newlib-2.5.0.20170922"

if [ ! -d ${GCC} ]; then

  ## Download the source code.
  if [ ! -f ${GCC}.tar.gz ]; then wget --continue http://ftp.gnu.org/gnu/gcc/${GCC}/${GCC}.tar.gz; fi
  if [ ! -f ${NEWLIB}.tar.gz ]; then wget --continue http://sourceware.org/pub/newlib/${NEWLIB}.tar.gz; fi

  ## Unpack the source code.
  rm -Rf ${GCC} && tar xfvz ${GCC}.tar.gz
